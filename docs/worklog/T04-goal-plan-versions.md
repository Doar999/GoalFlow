...
| [核心数据模型](../development/03-data-model.md) 第 1、2、3 节 | goals 及计划/路线族表结构 | **建议**（字段与状态为设计建议；其中第 7 节事务协议、T01 实测结论已确认） |
...
| [目标生命周期与结束实现基线](../development/17-goal-lifecycle-design.md) | 六个命令、状态机判定、goals 字段建议 | **建议**——实现前按决策规程确认，确认清单见第 8 节 |
...
| A2 | [17 号文档](../development/17-goal-lifecycle-design.md)的六个命令、状态机判定与 goals 七个新字段**按建议处理**：先完成第 8 节确认清单，仓库负责人确认后按确认稿实现，不抢先落码 | 17 号文档状态为"实施设计建议"；AGENTS.md 红线 2 | goals 表、生命周期命令、OpenAPI | 字段变更走契约 PR，不需 RFC |
| A3 | 暂停/恢复的实际排期与预算影响由 T05 计算、依赖影响由 T06 计算；T04 保证状态转换正确、递增 planning revision，并在响应中预留受影响实体列表字段（T06 表建立前返回空列表并显式标注） | [17 号文档](../development/17-goal-lifecycle-design.md)第 3 节；模块边界 | 跨模块接口 | 否 |
...
| A8 | pause 的依赖影响列表采用**接口占位 + 测试保护**：OpenAPI 现在即定义 `affected_dependencies`（并入 A5 契约 PR）；T04 提供 `compute_pause_impact(goal_id) -> list[AffectedDependency]` 接缝，stub 返回空列表并携带显式标记（如 `dependency_check: not_implemented`，枚举进 `contracts/`）；测试断言"空列表 + 标记存在"，把 A3 的"不能伪装成已检查"变成被保护的行为。T06 建 `task_dependencies` 后回填真实现，被依赖任务进入计算态 `blocked` | §8 原未决项；[17 号文档](../development/17-goal-lifecycle-design.md)第 3 节；模块边界 | pause 响应契约、goals 模块接口 | 否 |
| A9 | resume 的预算冲突判定采用**接缝契约 + 占位**：与 T05 负责人约定纯函数签名 `check_shared_weekly_budget(user_id, week, goal_demand) -> {ok, over_by_minutes, conflicts}`（不落库、不调模型，符合 [05-module-contracts](../development/05-module-contracts.md) 内部计算 Interface 风格）；T05 未合并前接缝返回 `not_available`，resume 照常完成状态转换并递增 revision。两层判定分工固定：**resume 检查是提示层，排期计算冲突原因码是权威层** | §8 原未决项；[17 号文档](../development/17-goal-lifecycle-design.md)第 3 节 | resume 流程、T05 接口签名 | 否 |
| A10 | `activate_goal` 并入 `activate_plan` **同一短事务**（方案 A）：单 Idempotency-Key，`expected_revision` 同携 goal revision 与 planning_revision，全成或全回滚，内部不拆分命令。依据：[17 号文档](../development/17-goal-lifecycle-design.md)第 2 节 draft→active 的附加条件即计划草稿校验；分事务会产生"goal=active 而计划未启用"的可观察中间态并需补偿逻辑与两套幂等键。**待仓库负责人批注后生效**；确认后回写 [12 号文档](../development/12-plan-engine.md)第 7 节与 17 号文档第 2 节 | §8 原未决项；[12 号文档](../development/12-plan-engine.md)第 7、8 节；T04 验收"重复启用只产生一个有效活动版本" | 生命周期命令、幂等键设计、T09 闭环 | 否（填补文档空白，非改已确认决策） |
...
- 未开始：代码零行。交接卡已建立。
- 第 8 节确认清单完成前不进入实现。
- 三项跨模块未决问题已决策（§4 A8–A10）：A8/A9 为接口占位方案，T04 可直接推进；A10 已选定方案 A，待仓库负责人批注后生效。

## 7. 验证结果

尚未运行任何命令。本节只记录真实执行过的命令与真实输出。

## 8. 未决问题

### D12 → 数据模型同步清单（需仓库负责人确认后回写 [03-data-model](../development/03-data-model.md)）

D12 已确认，但 [17 号实施设计](../development/17-goal-lifecycle-design.md) 仍是"建议"状态，以下各点确认前不得实现：

| # | 事项 | 现状与建议 | 需要谁决策 |
| --- | --- | --- | --- |
| 1 | goals 新增 7 字段：`paused_at`、`pause_reason`、`closed_at`、`closure_kind`、`closure_note`、`review_period`、`source_goal_id` | 17 第 6 节建议补充；03 的 goals 行尚未收录 | 仓库负责人 |
| 2 | `goals.status` 枚举与 CHECK：`draft/active/paused/completed/stopped` | D12 已定义状态集，03 未写枚举值；`completed` 仅 achievement 开放需应用层校验 | 仓库负责人 |
| 3 | 结束时成功标准逐条快照的存放位置 | 17 第 4 节要求快照**必需**，03 未给落点。建议 `goals.closure_criteria_snapshot_json`，或独立表（支持"完成并记录未达成项"的逐条对照展示） | 仓库负责人 |
| 4 | `goals.kind` 同步规则 | 形态由档案时间边界推导、随档案确认、可编辑；档案新版本改变时间边界时同步更新 goals.kind 并写审计（17 第 6 节） | 仓库负责人 |
| 5 | `review_period` 与版本化参数落点 | 默认 weekly；24h 撤销窗口同为"首版默认值"。两者都标注"版本化策略管理"，但仓库尚无策略参数表——挂在 strategy_version/policy_version 字段的取值规范需先定 | 仓库负责人 + T05 |
| 6 | blocked 是计算态 | 暂停使被依赖任务进入计算态 blocked，**不建列**；tasks 表不因 D12 新增任何字段（防顺手加） | 写入交接卡即可 |
| 7 | 派生关系记录方式 | 17 建议 `source_goal_id` + 审计；是否需要独立表（如后续要展示"派生自哪个目标"列表） | 仓库负责人 |

### 其他未决问题

原三项跨模块未决问题已于 2026-09-23 决策（见 §4 A8–A10），剩余待办如下：

| 事项 | 状态 | 后续动作 |
| --- | --- | --- |
| pause 依赖影响列表 | 已决策（A8）：接口占位 + 测试保护 | 契约 PR 定义 `affected_dependencies`；T06 合并后回填 `compute_pause_impact`，并打开 17 号文档"暂停后被依赖任务进入计算态 blocked"测试 |
| resume 预算冲突判定 | 已决策（A9）：接缝契约 + 占位 | 与 T05 负责人会签 `check_shared_weekly_budget` 签名；T05 合并后回填，并打开"恢复时共享预算已占满返回冲突"测试 |
| `activate_goal` 与 `activate_plan` 事务关系 | 已选定方案 A（合并单事务），**待仓库负责人批注生效** | 负责人批注后回写 12 号文档第 7 节、17 号文档第 2 节；幂等键按单 Key 设计落地 |
