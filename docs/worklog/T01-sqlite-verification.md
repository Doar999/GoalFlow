...
| 负责人 | （待填，数据负责人） |
| 状态 | 阻塞：验证套件已完成并通过，等待 [RFC 0003](../rfcs/0003-sqlite-as-primary-store.md) 接受后回写设计文档 |
| 更新日期 | 2026-09-23 |
| 相关 PR | #（待填） |
...
| 来源 | 引用内容 | 状态 |
| --- | --- | --- |
| [RFC 0003](../rfcs/0003-sqlite-as-primary-store.md) | 数据库选型、连接参数、并发协议收敛、部署硬约束 | 草案，**本工作包的结论以它被接受为前提** |
...
- 已完成：全部 39 条用例编写并通过；依赖调整（移除 pymysql，dev 组加入 sqlite-vec）；结论表填写完毕。
- 进行中：无。
- 未开始：设计文档回写。**它被 RFC 0003 卡着**——RFC 尚未被接受，此时改设计文档等于先斩后奏。RFC 接受后按其"接受后的回写清单"执行。
...
| 问题 | 影响 | 需要谁决策 |
| --- | --- | --- |
| RFC 0003 尚未被接受 | 本工作包的全部结论都以它为前提；未接受前不得回写设计文档 | 产品决策人与数据负责人 |
