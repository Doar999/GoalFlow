...
| 负责人 | 待指定（Agent 负责人） |
| 状态 | 进行中：T08 契约 #30 已合并；T14 作业 Interface #29 待评审 |
| 更新日期 | 2026-09-25 |
| 相关 PR | #27（交接卡）、T14 #26/#28（已合并）、#29（作业 Interface，待评审）、#30（T08 契约，已合并） |
...
T14 配置读取 Interface 由独立 PR #29 负责。澄清消息的表结构与 HTTP 形状已由 #30 交付；T04/T07 的业务接线还需在后续 PR 明确模块所有权与可修改路径。T08 业务实现 PR 不直接修改 T14 内部文件。
...
迁移 0008（对话与消息）、0009（模型调用）已随 #30 合并；不修改已合并迁移。#30 只交付表、ORM 和 HTTP 桩，保存消息、创建作业与模型计量业务实现仍待后续 PR。
...
| A4 | `model_calls` 已由迁移 0009 建表；用量缺失保持「未知」，不能记作 0。具体记录时点与失败调用在业务 PR 落地 | 03 号数据模型、Q07 建议质量要求、RFC 0004、#30 | 用量与数据库契约 | 契约已合并；业务语义待实现 |
...
- 已完成：T08 交接卡 #27、公共契约 #30 已合并；T02/T07 与 T14 #26/#28 已合并。#30 包含两条迁移、ORM、HTTP 桩、RFC 0004、导出的 OpenAPI 与前端类型。
- 进行中：T14 作业调用 Interface 与 `api_mode` 映射在独立 #29 待评审；代码和回归测试已通过本地门禁。
...
| 对话、消息与 `model_calls` 的表和 HTTP 契约已由 #30 交付，业务保存与 T07 作业接线未落地；T04 四个生成类端点仍是契约桩 | T08 的首个真实作业与 T09 接线需要明确业务 Interface | T04/T07/T08/T09 负责人 |
