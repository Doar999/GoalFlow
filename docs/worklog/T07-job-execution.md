...
PR-2，于 2026-09-23 在同一环境执行（分支 `feat/T07-job-core`，rebase 到合并了 #11 的 main `f9577fe` 之后重跑）：
...
$ bash scripts/test.sh
==> 后端测试（all）
251 passed, 1 warning in 33.46s
==> 前端测试（all）
 Test Files  3 passed (3)
      Tests  16 passed (16)
测试通过
...
PR-3，于 2026-09-23 在同一环境执行（分支 `feat/T07-job-api`，rebase 到合并了 #12 的 main `24c5fd4` 之后重跑）：
...
$ bash scripts/test.sh
==> 后端测试（all）
262 passed, 1 warning in 27.50s
==> 前端测试（all）
 Test Files  3 passed (3)
      Tests  16 passed (16)
测试通过
...
| SSE 在同步生成器里轮询，Starlette 把它放进线程池，每条连接最多占一个线程 5 分钟 | 同时订阅的连接数接近线程池上限（anyio 默认 40）时，其他同步接口会排队。个人工具的并发量通常够用，但需实测 | T13（部署时评估线程池大小或改为异步轮询） |
