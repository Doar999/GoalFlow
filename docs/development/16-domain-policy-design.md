...
| 计划生成 | 里程碑验收方式、诊断阶段位置、依赖无环、任务时长上限、依赖的解锁条件 | [计划引擎](12-plan-engine.md) |
| 任务批次 | 复习任务的引用完整性、复习间隔、判分标准齐备 | [计划引擎](12-plan-engine.md) 滚动细化；复习间隔跨批次判定，依赖快照中的学习单元与复习历史 |
...
健身领域的依赖解锁条件作为一条 `scope=plan`、`severity=reject` 的约束落地：`fitness_dependency_outcome`——健身领域产出的 `task_dependencies` 不得取 `required_outcome=verification_passed`，只能取"执行完成"。命中即拒绝整个候选，与阻断集合同样不做"删掉违规边后继续使用"的局部修补。产品依据见 [三类领域专业规则](../product/11-domain-rules.md) 第 4 节。
...
- 复习任务缺少被复习单元引用时被拒绝。
- 第一个七日批次引入的学习单元，其 14 天复习在第二或第三个批次中按间隔正确落位；快照缺少复习历史时该约束应当失败而不是静默放行。
- 健康限制命中时整个候选被拒绝，而不是删除违规任务后放行。
- 健身领域生成 `required_outcome=verification_passed` 的依赖边时，`fitness_dependency_outcome` 拒绝整个候选；数据库中不存在这样的健身依赖边。
